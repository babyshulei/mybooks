...
![1591256662559](.\images\ssl.png)

#### 1. 客户端发出请求（ClientHello）

在这一步，客户端主要向服务器提供以下信息：

（1） 支持的**协议版本**，比如TLS 1.0版。

（2） 一个**客户端生成的随机数**，稍后用于生成"对话密钥"。

（3） 支持的**加密方法**，比如RSA公钥加密。

（4） 支持的**压缩方法**。

#### 2. 服务器回应（ServerHello）

服务器收到客户端请求后，向客户端发出响应。服务器回应包含以下内容：

（1） 确认使用的**加密通信协议版本**，比如TLS 1.0版本。如果浏览器与服务器支持的版本不一致，服务器关闭加密通信。

（2） 一个**服务器生成的随机数**，稍后用于生成"对话密钥"。

（3） 确认使用的**加密方法**，比如RSA公钥加密。
...
如果证书没有问题，客户端就会从证书中**取出服务器的公钥**。然后，向服务器发送下面三项信息。

（1） 一个**随机数**。该随机数用服务器公钥加密，防止被窃听。

这里是握手阶段出现的第三个随机数，又称“pre-master key”。

有了它以后，客户端和服务器就同时有了**三个**随机数，接着双方就用事先商定的加密方法，各自生成本次会话所用的同一把"会话密钥"。

（2） **编码改变通知**，表示随后的信息都将用双方商定的加密方法和密钥发送。

（3） **客户端握手结束通知**，表示客户端的握手阶段已经结束。这一项同时也是**前面发送的所有内容的hash值**，用来供服务器校验。

> 为什么要用三个随机数来生成“会话密钥”？
>
> 因为 SSL 协议不信任每个主机都能产生完全随机的随机数。
>
> 一个伪随机可能完全不随机，但是三个伪随机就十分接近随机了。
...
至此，整个握手阶段结束。接下来，客户端与服务器进入加密通信，就完全是使用普通的HTTP协议，只不过用"会话密钥"加密内容。



### 安全术语

1. 公钥/私钥（public/private keys）

	a. 用于在浏览器和服务器之间签名和解密信息
	b. 是一对非常大的数字，用作加密和解密密钥，并且共享特殊的数学关系
	c. 常见系统是 [RSA 密码系统](https://en.wikipedia.org/wiki/RSA_(cryptosystem))
	d. 公钥用于加密消息，并且只能使用相应的私钥对消息进行解密。Web 服务器将公钥发布给全世界，客户端（如 Web 服务器）使用该公钥将安全通道引导至您的服务器



2. 证书颁发机构（certificate authority, CA）

	a. 是公钥和公共 DNS 名称（如“www.foobar.com”）之间映射的组织
	b. 作用：
		i. 客户端如何知道特定的公钥是否是 [www.foorbar.com](http://www.foorbar.com) 的真实公钥？
		ii. CA 通过使用自己的私钥对网站的公共密钥进行[加密签名](https://en.wikipedia.org/wiki/RSA_(cryptosystem)#Signing_messages)，来证实特定密钥是特定站点的真实密钥。此签名在计算上是无法伪造的
		iii. 浏览器（和其他客户端）维护信任锚存储库（trust anchor stores），其中包含知名 CA 拥有的公共密钥，并使用这些公共密钥对 CA 的签名进行加密验证（crytographically verify）



3. 证书签名请求（certificate signing request, CSR）

	a. 是一种数据格式，将公钥和一些有关拥有密钥的实体的元数据捆绑在一起
	b. 客户端不解释 CSR，CA 解释
	c. 当希望让 CA 证实 Web 服务器的公共密钥时，可以向 CA 发送 CSR。CA 验证 CSR 中的信息，并使用它来生成证书。然后 CA 向您发送最终证书，您在 Web 服务器上安装该证书（或更可能是证书链）和私钥
...
[安全术语](https://developers.google.com/web/fundamentals/security/encrypt-in-transit/intro-to-security-terminology)
